...
public class HighPress: MonoBehaviour{
	...
	void Awake(){
		...
		foreach(SpriteRenderer rend in my_renderer){
			if(rend.sprite != my_renderer[12].sprite) rend.enabled = false;
		}		
	}
	...
	void FixedUpdate () {
		...
		if((Mathf.Abs (mega_man.position.x - this.transform.position.x)) < 1.19f || going_down){
			if (starting_position || going_down) {
				frames_passed++;
				if(frames_passed < 5f) return; // stall slightly before going down
		...
		if (!starting_position && !going_down) {
			frames_passed++;
			if(frames_passed < 75f) return; // stall for 1.5sec when press hits ground
			Vector3 temp_pos = this.transform.position;
			temp_pos.y += upward_vel;
			if(temp_pos.y >= 3.293f){ // if press is back at starting location
				temp_pos.y = 3.293f;
				starting_position = true;
				frames_passed = 0f;
			}
		...
		if (y < 3.25) my_renderer [0].enabled = true;
		else my_renderer [0].enabled = false;
		if (y < 2.98) my_renderer [1].enabled = true;
		else my_renderer [1].enabled = false;
		if (y < 2.79) my_renderer [2].enabled = true;
		else my_renderer [2].enabled = false;
		if (y < 2.55) my_renderer [3].enabled = true;
		else my_renderer [3].enabled = false;
		if (y < 2.31) my_renderer [4].enabled = true;
		else my_renderer [4].enabled = false;
		if (y < 2.07) my_renderer [5].enabled = true;
		else my_renderer [5].enabled = false;
		if (y < 1.83) my_renderer [6].enabled = true;
		else my_renderer [6].enabled = false;
		if (y < 1.598) my_renderer [7].enabled = true;
		else my_renderer [7].enabled = false;
		if (y < 1.321) my_renderer [8].enabled = true;
		else my_renderer [8].enabled = false;
		if (y < 1.054) my_renderer [9].enabled = true;
		else my_renderer [9].enabled = false;
		if (y < .877) my_renderer [10].enabled = true;
		else my_renderer [10].enabled = false;
		if (y < .622) my_renderer [11].enabled = true;
		else my_renderer [11].enabled = false;
	}
